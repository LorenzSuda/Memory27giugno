    ...
    private void SelectedCard(InteractiveCard card, bool selected)
    {
        ...
        else if (selectedCard2 == null && card != selectedCard1 && selected) //se la seconda carta è diversa dalla prima
        {
            ...
            if (selectedCard1.Compare(selectedCard2)) //qui confronti le due carte, 2 texture! suppongo vedi dopo...
            {
                ...
                {
                    //You win!!!
                    Vector3 screenCenter = new Vector3(Screen.width / 2, Screen.height / 2, Camera.main.nearClipPlane + 1);
                    Vector3 worldPosition = Camera.main.ScreenToWorldPoint(screenCenter);
                    Instantiate(ninja, worldPosition, Quaternion.identity);
                    
                    for (int i = 0; i < 10; i++)
                    {
                        // Genera una posizione casuale sullo schermo
                        Vector3 randomSparkPosition = new Vector3(
                            Random.Range(0, Screen.width),
                            Random.Range(0, Screen.height),
                            Camera.main.nearClipPlane + 1
                        );
                        
                        Vector3 sparkPosition = Camera.main.ScreenToWorldPoint(randomSparkPosition);
                        
                        Instantiate(electricalSparksEffect, sparkPosition, Quaternion.identity);
                    }
